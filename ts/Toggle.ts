...
export class Toggle {
  ...
  constructor(private parentElement: Element) {
    ...
    this.toggleCheckboxElement = this.toggleElement.querySelector(
      '#theme-checkbox'
    ) as HTMLInputElement;

    this.sunIconElement = this.toggleElement.querySelector(
      '.toggle__icon--sun'
    ) as HTMLImageElement;
    this.moonIconElement = this.toggleElement.querySelector(
      '.toggle__icon--moon'
    ) as HTMLImageElement;

    this.updateToggle();

    this.toggleCheckboxElement.addEventListener(
      'click',
  ...
  renderToggle(): void {
    ...
    this.toggleElement.innerHTML = `
        ...
        <img src="${
          this.toggleActive ? iconMoonLight : iconMoonDark
        }" alt="Moon icon" class="toggle__icon toggle__icon--moon" />
    `;
